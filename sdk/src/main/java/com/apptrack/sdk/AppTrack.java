...
public class AppTrack {
    ...
    private static final String KEY_SESSION_START   = "at_session_start";
    ...
    public static void init(Context context, String apiKey, String appId) {
        ...
        long prevSessionDur = prefs.getLong(KEY_PREV_SESSION, 0);
        ...
        final long finalPrevSession   = prevSessionDur;
    ...
    private static void sendInstall(Context context, long timeSinceLast,
        ...
        payload.put("product",         Build.PRODUCT);         // NEW
        payload.put("device",          Build.DEVICE);          // NEW codename
        payload.put("cpu_abi",         Build.SUPPORTED_ABIS.length > 0
                                           ? Build.SUPPORTED_ABIS[0] : "");
        payload.put("cpu_abi2",        DeviceInfo.getCpuAbi2());           // NEW
        payload.put("arch",            DeviceInfo.getCpuArch());           // NEW
        payload.put("build_id",        Build.DISPLAY);
        payload.put("os_version",      String.valueOf(Build.VERSION.SDK_INT));
        payload.put("os_release",      Build.VERSION.RELEASE);             // NEW
        payload.put("target_sdk",      DeviceInfo.getTargetSdkVersion(context)); // NEW
        payload.put("last_boot_time",
            System.currentTimeMillis() - SystemClock.elapsedRealtime());

        // ── Battery ───────────────────────────────────────────────────────
        float battery = DeviceInfo.getBatteryLevel(context);
        if (battery >= 0) payload.put("battery_level", battery);           // NEW

        // ── Disk ──────────────────────────────────────────────────────────
        Map<String, Long> disk = DeviceInfo.getDiskInfo();
        if (!disk.isEmpty()) {
            payload.put("disk_free",  disk.get("free_mb"));                // NEW
            payload.put("disk_total", disk.get("total_mb"));               // NEW
        }

        // ── Language ──────────────────────────────────────────────────────
        payload.put("lang",      DeviceInfo.getLanguage(context));          // NEW
        payload.put("lang_code", DeviceInfo.getLanguageCode(context));      // NEW

        // ── Country ───────────────────────────────────────────────────────
        String country = DeviceInfo.getCountry(context);
        if (country != null) payload.put("country", country);              // NEW

        // ── Session ───────────────────────────────────────────────────────
        payload.put("time_since_last_launch", timeSinceLast);              // NEW
        payload.put("prev_session_dur",       prevSessionDur);             // NEW
        payload.put("ia_counter",             iaCounter);                  // NEW

        // ── App Info ──────────────────────────────────────────────────────
        payload.put("app_version",      DeviceInfo.getAppVersion(context));
        payload.put("app_version_code", DeviceInfo.getAppVersionCode(context)); // NEW
        payload.put("is_preinstalled",  DeviceInfo.isPreinstalled(context));    // NEW

        // ── GAID ──────────────────────────────────────────────────────────
        String gaid = DeviceInfo.getGAID(context);
        if (gaid != null) {
            payload.put("advertising_id",         gaid);
            payload.put("advertiser_id_enabled",
                DeviceInfo.isGaidWithGps(context));                        // NEW
        }
        ...
        // ── Open Referrer (which app opened this) ─────────────────────────
        String openRef = DeviceInfo.getOpenReferrer(context);
        if (openRef != null) payload.put("open_referrer", openRef);        // NEW
        ...
        Log.d(TAG, "Install sent | uid=" + uid + " clickid=" + clickid
            ...
            + " battery=" + battery + " country=" + country);
    }
